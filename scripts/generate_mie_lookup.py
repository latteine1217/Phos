#!/usr/bin/env python3
"""
Mie 散射查表生成腳本 (Phase 5.5 - 高密度版本)

離線計算真實 Mie 散射參數，壓縮為 (σ, κ, ρ, η) 雙段核查表

版本更新:
    v1: 3 波長 × 7 ISO = 21 格點
    v2: 10 波長 × 20 ISO = 200 格點 (Phase 5.5)

依賴:
    pip install miepython numpy scipy

使用:
    python3 generate_mie_lookup.py
    
輸出:
    ../data/mie_lookup_table_v3.npz
"""

import numpy as np
import miepython
from scipy.optimize import minimize
from scipy.stats import lognorm
import time
import os

# ============================================================
# 1. 物理參數定義
# ============================================================

# Phase 5.5: 增加查表密度 (v1: 3×7, v2: 10×20)
WAVELENGTHS = np.linspace(400, 700, 10)  # nm (10 點: 400, 433, 467, ..., 700)
ISO_VALUES = [50, 100, 125, 160, 200, 250, 320, 400, 500, 640, 800, 1000, 1250, 1600, 2000, 2500, 3200, 4000, 5000, 6400]

# 粒徑分布 (對數常態參數, μm)
# Phase 5.5: 擴充至 20 個 ISO 值 (線性插值 log(粒徑) vs log(ISO))
PARTICLE_DISTRIBUTIONS = {
    50:   {'mean': 0.7,  'std': 0.28},
    100:  {'mean': 0.8,  'std': 0.30},
    125:  {'mean': 0.85, 'std': 0.32},
    160:  {'mean': 0.9,  'std': 0.33},
    200:  {'mean': 1.0,  'std': 0.35},
    250:  {'mean': 1.05, 'std': 0.36},
    320:  {'mean': 1.1,  'std': 0.38},
    400:  {'mean': 1.2,  'std': 0.40},
    500:  {'mean': 1.3,  'std': 0.42},
    640:  {'mean': 1.4,  'std': 0.43},
    800:  {'mean': 1.5,  'std': 0.45},
    1000: {'mean': 1.6,  'std': 0.46},
    1250: {'mean': 1.7,  'std': 0.48},
    1600: {'mean': 1.8,  'std': 0.50},
    2000: {'mean': 1.95, 'std': 0.52},
    2500: {'mean': 2.1,  'std': 0.53},
    3200: {'mean': 2.2,  'std': 0.55},
    4000: {'mean': 2.35, 'std': 0.57},
    5000: {'mean': 2.45, 'std': 0.58},
    6400: {'mean': 2.5,  'std': 0.60}
}

# 折射率
N_GELATIN = 1.50  # 明膠介質

def n_AgBr_vacuum(wavelength_nm):
    """
    AgBr 折射率（相對真空/空氣）
    
    基於 Cauchy 色散公式，擬合 Palik (1985) 數據
    適用範圍: 400-700 nm (可見光)
    
    Args:
        wavelength_nm: 波長 (nm)
    
    Returns:
        n_AgBr: 折射率（相對真空）
    
    參考文獻:
        Palik, E. D. (1985). Handbook of Optical Constants of Solids.
        Academic Press. Vol. 1, pp. 749-763.
        
    數值驗證 (Cauchy 擬合 RMSE=0.0142):
        λ=450nm: n=2.350 (文獻: 2.370, 誤差 0.85%)
        λ=550nm: n=2.247 (文獻: 2.250, 誤差 0.12%)
        λ=650nm: n=2.188 (文獻: 2.180, 誤差 0.37%)
    """
    λ_um = wavelength_nm / 1000.0
    
    # Cauchy 係數（最小二乘擬合 Palik 1985 數據，400-700nm）
    A = 2.0393
    B = 0.0629  # μm² 單位，RMSE=0.0142
    
    n = A + B / (λ_um ** 2)
    
    return n

def relative_refractive_index(wavelength_nm):
    """
    相對折射率 m = n_AgBr / n_gelatin
    
    Mie 理論要求的是粒子折射率相對於**介質**折射率
    
    物理情境:
        - 粒子: 銀鹵化物晶粒 (AgBr)
        - 介質: 明膠 (gelatin, 含水 ~10-15%)
        - n_gelatin ≈ 1.50 @ 589nm (濕明膠)
    
    Args:
        wavelength_nm: 波長 (nm)
    
    Returns:
        m: 相對折射率（無單位）
    
    參考文獻:
        Bohren & Huffman (1983), Absorption and Scattering of Light
        by Small Particles, Chapter 4.
    """
    n_particle = n_AgBr_vacuum(wavelength_nm)
    n_medium = N_GELATIN  # 1.50
    
    m = n_particle / n_medium
    
    return m

# ============================================================
# 2. Mie 散射計算
# ============================================================

def compute_mie_phase_function(wavelength_nm, particle_diameter_um, angles_deg):
    """
    計算單顆粒的 Mie 相位函數
    
    Args:
        wavelength_nm: 波長 (nm)
        particle_diameter_um: 粒徑 (μm)
        angles_deg: 角度陣列 (度)
    
    Returns:
        P(θ): 相位函數（未正規化）
    """
    # 尺寸參數 x = πd/λ
    x = np.pi * particle_diameter_um / (wavelength_nm / 1000)
    
    # 相對折射率
    m = relative_refractive_index(wavelength_nm)
    
    # 計算散射矩陣元素 (S1, S2)
    mu = np.cos(np.deg2rad(angles_deg))
    
    try:
        s1, s2 = miepython.S1_S2(m, x, mu)
    except Exception as e:
        print(f"    ⚠️  Mie 計算警告: λ={wavelength_nm}nm, d={particle_diameter_um:.2f}μm, x={x:.2f}, m={m:.3f}")
        print(f"        {e}")
        # 返回各向同性散射作為 fallback
        return np.ones_like(angles_deg)
    
    # 相位函數 P(θ) ∝ |S1|² + |S2|²
    phase = (np.abs(s1)**2 + np.abs(s2)**2)
    
    return phase

def compute_polydisperse_phase(wavelength_nm, iso, angles_deg, n_samples=30):
    """
    計算粒徑分布加權的平均相位函數
    
    Args:
        wavelength_nm: 波長 (nm)
        iso: 感光度
        angles_deg: 角度陣列 (度)
        n_samples: 粒徑採樣點數
    
    Returns:
        <P(θ)>: 粒徑分布加權平均相位函數
    """
    params = PARTICLE_DISTRIBUTIONS[iso]
    mean_um = params['mean']
    std_um = params['std']
    
    # 對數常態分布採樣
    scale = mean_um
    s = std_um / mean_um  # shape parameter
    dist = lognorm(s=s, scale=scale)
    
    # 粒徑範圍: [0.3 μm, 4 μm]
    diameters = np.linspace(0.3, 4.0, n_samples)
    weights = dist.pdf(diameters)
    weights = weights / (np.sum(weights) + 1e-10)  # 正規化
    
    # 加權平均相位函數
    phase_avg = np.zeros_like(angles_deg, dtype=float)
    for d, w in zip(diameters, weights):
        phase = compute_mie_phase_function(wavelength_nm, d, angles_deg)
        phase_avg += w * phase
    
    # 正規化: ∫ P(θ) sin(θ) dθ = 2 (積分為 4π / 2π)
    theta_rad = np.deg2rad(angles_deg)
    norm_factor = np.trapz(phase_avg * np.sin(theta_rad), theta_rad)
    phase_avg = phase_avg / (norm_factor + 1e-10)
    
    return phase_avg

# ============================================================
# 3. 角度 → 空間 PSF 轉換
# ============================================================

def phase_to_spatial_psf(phase_function, angles_deg, z_eff_um=500.0, 
                        pixel_size_um=12.0, max_radius_px=150):
    """
    將角度相位函數轉換為空間 PSF
    
    使用小角度近似與投影幾何：
    - 散射角 θ → 影像平面位移 r ≈ z_eff × tan(θ)
    - PSF(r) ∝ P(θ) with θ = arctan(r/z_eff)
    - z_eff 為等效積分深度 (含多次散射與 halation 效應)
    
    Args:
        phase_function: P(θ) 陣列
        angles_deg: 對應角度 (度)
        z_eff_um: 等效散射深度 (μm, 預設 500μm 以匹配實際光暈尺度)
        pixel_size_um: 像素尺寸 (μm/px)
        max_radius_px: 最大半徑 (像素)
    
    Returns:
        r_px, PSF(r): 徑向 PSF 分布
    """
    # 創建均勻空間網格（像素）
    r_grid = np.linspace(0, max_radius_px, 300)
    r_grid_um = r_grid * pixel_size_um
    
    # 反推對應的散射角: θ ≈ arctan(r/z_eff) ≈ r/z_eff (小角度)
    theta_rad = np.arctan(r_grid_um / z_eff_um)
    theta_deg = np.rad2deg(theta_rad)
    
    # 插值相位函數到空間網格
    psf_grid = np.interp(theta_deg, angles_deg, phase_function, right=0)
    
    # 正規化: ∫ PSF(r) 2πr dr = 1
    norm_factor = np.trapz(psf_grid * r_grid, r_grid) * 2 * np.pi
    psf_grid = psf_grid / (norm_factor + 1e-10)
    
    return r_grid, psf_grid

# ============================================================
# 4. 雙段核參數擬合
# ============================================================

def dual_kernel(r, sigma, kappa, rho):
    """
    雙段核模型
    K(r) = ρ·G(r; σ) + (1-ρ)·E(r; κ)
    """
    gaussian = np.exp(-r**2 / (2 * sigma**2 + 1e-10))
    exponential = np.exp(-r / (kappa + 1e-10))
    kernel = rho * gaussian + (1 - rho) * exponential
    
    # 正規化
    norm = np.trapz(kernel * r, r) * 2 * np.pi
    return kernel / (norm + 1e-10)

def fit_dual_kernel(r, psf_target):
    """
    擬合雙段核參數 (σ, κ, ρ)
    
    Returns:
        (sigma, kappa, rho, rmse)
    """
    # 初始猜測（基於 Phase 1 參數）
    sigma_init = 20.0  # px
    kappa_init = 30.0  # px (1.5σ)
    rho_init = 0.75
    
    def objective(params):
        sigma, kappa, rho = params
        if sigma <= 0 or kappa <= 0:
            return 1e10  # 懲罰非法值
        psf_model = dual_kernel(r, sigma, kappa, rho)
        # RMSE
        return np.sqrt(np.mean((psf_target - psf_model)**2))
    
    # 約束
    bounds = [(5, 50), (10, 100), (0.5, 0.95)]  # (σ, κ, ρ)
    
    try:
        result = minimize(objective, [sigma_init, kappa_init, rho_init], 
                         bounds=bounds, method='L-BFGS-B', 
                         options={'maxiter': 100})
        sigma, kappa, rho = result.x
        rmse = result.fun
    except Exception as e:
        print(f"    ⚠️  擬合失敗: {e}, 使用預設值")
        sigma, kappa, rho = sigma_init, kappa_init, rho_init
        rmse = objective([sigma, kappa, rho])
    
    return sigma, kappa, rho, rmse

# ============================================================
# 5. 能量係數計算
# ============================================================

def compute_energy_fraction(wavelength_nm, iso):
    """
    計算散射能量分數 η(λ, ISO)
    
    基於 Mie 散射效率 Q_scat
    """
    params = PARTICLE_DISTRIBUTIONS[iso]
    mean_um = params['mean']
    
    # 尺寸參數
    x = np.pi * mean_um / (wavelength_nm / 1000)
    m = relative_refractive_index(wavelength_nm)
    
    try:
        # Mie 散射效率 Q_scat (使用 efficiencies 函數)
        d_m = mean_um * 1e-6  # 直徑 (μm → m)
        lambda0_m = wavelength_nm * 1e-9  # 波長 (nm → m)
        qext, qsca, qback, g = miepython.efficiencies(m, d_m, lambda0_m)  # m 已是相對折射率，無需 n_env
    except Exception as e:
        print(f"    ⚠️  能量計算警告: λ={wavelength_nm}nm, ISO={iso}, x={x:.2f}, m={m:.3f}")
        print(f"        {e}")
        # Fallback to Rayleigh approximation
        qsca = (8.0/3.0) * x**4 * abs((m**2 - 1)/(m**2 + 2))**2
    
    # 正規化：綠光 (550nm, ISO=400) 為基準
    try:
        d_ref_m = 1.2e-6  # ISO 400 → d=1.2μm (轉換為 m)
        lambda_ref_m = 550e-9  # 550nm (轉換為 m)
        m_ref = relative_refractive_index(550)
        _, qsca_ref, _, _ = miepython.efficiencies(m_ref, d_ref_m, lambda_ref_m)  # m_ref 已是相對折射率
    except:
        qsca_ref = 1.0
    
    eta = qsca / (qsca_ref + 1e-10)
    
    return eta

# ============================================================
# 6. 主生成流程
# ============================================================

def generate_lookup_table():
    """生成完整查表"""
    print("=" * 70)
    print("  Mie 散射查表生成 (Phase 5.5 - 高密度版本)")
    print("=" * 70)
    print(f"\n參數設定:")
    print(f"  波長: {len(WAVELENGTHS)} 點 ({WAVELENGTHS.min():.0f}-{WAVELENGTHS.max():.0f} nm)")
    print(f"  ISO 值: {len(ISO_VALUES)} 點 ({min(ISO_VALUES)}-{max(ISO_VALUES)})")
    print(f"  總格點數: {len(WAVELENGTHS)} × {len(ISO_VALUES)} = {len(WAVELENGTHS) * len(ISO_VALUES)}")
    print(f"  粒徑分布: 對數常態 (mean: 0.7-2.5 μm)")
    print(f"  相對折射率: m ≈ {relative_refractive_index(550):.3f} @ 550nm")
    print()
    
    # 初始化表格
    n_wavelengths = len(WAVELENGTHS)
    n_isos = len(ISO_VALUES)
    
    table_sigma = np.zeros((n_wavelengths, n_isos))
    table_kappa = np.zeros((n_wavelengths, n_isos))
    table_rho = np.zeros((n_wavelengths, n_isos))
    table_eta = np.zeros((n_wavelengths, n_isos))
    
    # 角度網格 (小角區域)
    angles_deg = np.linspace(0.1, 20, 150)  # 0.1° ~ 20°
    
    # 統計資訊
    total_cases = n_wavelengths * n_isos
    case_count = 0
    start_time = time.time()
    
    # 遍歷所有 (λ, ISO) 組合
    for i, wavelength in enumerate(WAVELENGTHS):
        for j, iso in enumerate(ISO_VALUES):
            case_count += 1
            print(f"\n[{case_count}/{total_cases}] λ={wavelength}nm, ISO={iso}")
            print("-" * 70)
            
            case_start = time.time()
            
            # 1. 計算多粒徑平均相位函數
            print("  [1/4] 計算 Mie 相位函數...", end=" ", flush=True)
            t0 = time.time()
            phase = compute_polydisperse_phase(wavelength, iso, angles_deg)
            print(f"✓ ({time.time()-t0:.2f}s)")
            
            # 2. 轉換為空間 PSF
            print("  [2/4] 轉換為空間 PSF...", end=" ", flush=True)
            t0 = time.time()
            r, psf = phase_to_spatial_psf(phase, angles_deg)
            print(f"✓ ({time.time()-t0:.2f}s)")
            
            # 3. 擬合雙段核
            print("  [3/4] 擬合雙段核參數...", end=" ", flush=True)
            t0 = time.time()
            sigma, kappa, rho, rmse = fit_dual_kernel(r, psf)
            print(f"✓ ({time.time()-t0:.2f}s)")
            
            # 4. 計算能量分數
            print("  [4/4] 計算能量分數...", end=" ", flush=True)
            t0 = time.time()
            eta = compute_energy_fraction(wavelength, iso)
            print(f"✓ ({time.time()-t0:.2f}s)")
            
            # 儲存結果
            table_sigma[i, j] = sigma
            table_kappa[i, j] = kappa
            table_rho[i, j] = rho
            table_eta[i, j] = eta
            
            case_time = time.time() - case_start
            print(f"\n  ✅ 結果: σ={sigma:.2f}px, κ={kappa:.2f}px, ρ={rho:.3f}, η={eta:.3f}")
            print(f"     RMSE={rmse:.4f}, 耗時={case_time:.2f}s")
    
    total_time = time.time() - start_time
    
    # 封裝為字典
    lookup_table = {
        'wavelengths': WAVELENGTHS,
        'iso_values': np.array(ISO_VALUES),
        'sigma': table_sigma,
        'kappa': table_kappa,
        'rho': table_rho,
        'eta': table_eta,
        'metadata': {
            'z_eff_um': 500.0,  # Phase 5.2 修正值
            'pixel_size_um': 12.0,
            'n_gelatin': N_GELATIN,
            'particle_distributions': PARTICLE_DISTRIBUTIONS,
            'version': '3.0',  # TASK-010: 修正折射率
            'date': '2025-12-20',
            'resolution': f'{len(WAVELENGTHS)}x{len(ISO_VALUES)} (高密度)',
            'library': f'miepython {miepython.__version__}',
            'generation_time_sec': total_time
        }
    }
    
    # 儲存
    output_dir = os.path.join(os.path.dirname(__file__), '..', 'data')
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, 'mie_lookup_table_v3.npz')
    
    np.savez_compressed(output_path, **lookup_table)
    
    file_size = os.path.getsize(output_path) / 1024
    
    print("\n" + "=" * 70)
    print("  生成完成！")
    print("=" * 70)
    print(f"✅ 查表已儲存: {output_path}")
    print(f"   檔案大小: {file_size:.2f} KB")
    print(f"   總耗時: {total_time:.1f}s ({total_time/total_cases:.1f}s/case)")
    print()
    
    # 顯示統計摘要
    print("統計摘要:")
    print(f"  σ 範圍: {np.min(table_sigma):.2f} ~ {np.max(table_sigma):.2f} px")
    print(f"  κ 範圍: {np.min(table_kappa):.2f} ~ {np.max(table_kappa):.2f} px")
    print(f"  ρ 範圍: {np.min(table_rho):.3f} ~ {np.max(table_rho):.3f}")
    print(f"  η 範圍: {np.min(table_eta):.3f} ~ {np.max(table_eta):.3f}")
    print()
    
    # 驗證 η 的波長依賴性 (Phase 5.5: 找最接近 450nm 和 650nm 的索引)
    idx_450 = np.argmin(np.abs(WAVELENGTHS - 450))
    idx_650 = np.argmin(np.abs(WAVELENGTHS - 650))
    
    eta_450 = table_eta[idx_450, :]  # 藍光 @ all ISOs
    eta_650 = table_eta[idx_650, :]  # 紅光 @ all ISOs
    ratio_avg = np.mean(eta_450 / (eta_650 + 1e-10))
    print(f"驗證: η({WAVELENGTHS[idx_450]:.0f}nm)/η({WAVELENGTHS[idx_650]:.0f}nm) 平均比例 = {ratio_avg:.2f}x")
    print(f"      (Phase 1 經驗公式: 3.62x, 預期 Mie 較低 ~2-3x)")
    print(f"      (Phase 5.2 結果: 0.14x, Mie 振盪效應)")
    
    return lookup_table

# ============================================================
# 7. 主程式
# ============================================================

if __name__ == '__main__':
    try:
        table = generate_lookup_table()
        print("\n🎉 Mie 查表生成成功！")
        print("   可繼續 Phase 5.3: 整合到 Phos_0.3.0.py")
    except KeyboardInterrupt:
        print("\n\n⚠️  用戶中斷，查表生成未完成")
    except Exception as e:
        print(f"\n\n❌ 錯誤: {e}")
        import traceback
        traceback.print_exc()
