...
from film_models import get_film_profile, BloomParams, GrainParams
# Phase 1 Task 2: 使用統一的 apply_bloom() 替代 apply_bloom_optimized()
# Phase 1 Task 3: 使用統一的 generate_grain() 替代 generate_grain_optimized()
from Phos import apply_bloom, generate_grain
...
class TestPerformance:
    ...
    def test_grain_generation_speed(self, sample_image_float, benchmark):
        ...
        result = benchmark(generate_grain, sample_image_float, grain_params, 0.5)
        assert result.shape == sample_image_float.shape
    
    def test_bloom_speed(self, sample_image_float):
        """測試光暈效果速度"""
        # Phase 1 Task 2: 更新為使用 apply_bloom() 統一介面
        bloom_params = BloomParams(
            mode="artistic",
            sensitivity=0.5,
            radius=20,
            artistic_strength=0.5,
            artistic_base=0.05
        )
    ...
    def test_memory_efficiency(self, sample_image_float):
        ...
        grain_params = GrainParams(mode="artistic", intensity=0.18)
        for _ in range(50):
            _ = generate_grain(sample_image_float, grain_params, 0.5)
...
class TestScalability:
    ...
    def test_processing_scales_linearly(self, size):
        ...
        grain_params = GrainParams(mode="artistic", intensity=0.18)
        start = time.time()
        result = generate_grain(image, grain_params, 0.5)
